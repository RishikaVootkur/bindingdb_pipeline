...
An end-to-end data engineering pipeline that ingests, transforms, validates, and loads large-scale small molecule-protein binding interaction data from ChEMBL into a normalized PostgreSQL database, with analytical SQL queries for downstream analysis.
...
- **interactions** — binding measurements linking targets to compounds (IC50, Ki, Kd, EC50, pChEMBL value, potency category, assay details)
...
**1. Clone the repo**
```bash
git clone https://github.com/RishikaVootkur/bindingdb_pipeline.git
...
- **Normalized schema** — targets, compounds, and interactions stored in separate tables linked by foreign keys, avoiding data duplication
- **Validation layer** — pipeline halts automatically if data quality checks fail, preventing bad data from reaching the database
- **Caching** — raw API responses saved locally so the pipeline re-runs without re-fetching data
- **Indexed queries** — indexes on target_id, affinity_value_nm, and affinity_category ensure fast analytical queries at scale